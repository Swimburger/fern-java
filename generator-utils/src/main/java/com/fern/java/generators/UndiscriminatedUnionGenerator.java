...
public final class UndiscriminatedUnionGenerator extends AbstractFileGenerator {
    ...
    private List<MethodSpec> getConstructors() {
        ...
        for (int i = 0; i < undiscriminatedUnion.getMembers().size(); ++i) {
            ...
            constructors.add(MethodSpec.constructorBuilder()
                    ...
                    .addStatement("this.$L = $L", TYPE_FIELD_NAME, i)
                    .build());
        }
        return constructors;
    }
    ...
    private List<MethodSpec> getStaticFactories() {
        ...
        for (int i = 0; i < undiscriminatedUnion.getMembers().size(); ++i) {
            ...
            staticFactories.add(MethodSpec.methodBuilder(STATIC_FACTORY_METHOD_NAME)
                    ...
                    .addParameter(memberTypeNames.get(member), VALUE_FIELD_SPEC.name)
                    .addStatement("this($L)", VALUE_FIELD_SPEC.name)
                    .build());
        }
    ...
    private TypeSpec getDeserializer() {
        TypeSpec.Builder deserializerBuilder = TypeSpec.classBuilder(className.nestedClass("Deserializer"))
                .addModifiers(Modifier.STATIC, Modifier.FINAL)
                .addSuperinterface(ParameterizedTypeName.get(StdDeserializer.class, Object.class))
        ...
        for (int i = 0; i < undiscriminatedUnion.getMembers().size(); ++i) {
            ...
            deserializeMethod.addStatement("throw new $T($S)", JsonParseException.class, "Failed to deserialize");
        }
